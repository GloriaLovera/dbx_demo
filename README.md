...
```
dbx execute --cluster-name=<name of interactive cluster> --job=dbx_ci_cd_demo-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=dbx_ci_cd_demo-sample-integration-test --files-only
dbx launch --job=dbx_ci_cd_demo-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `dbx_ci_cd_demo/__init__.py` file and tag the current code version:
```
